  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
       <div className={`w-full max-w-xs p-5 rounded-[32px] shadow-2xl transform transition-all scale-100 border ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-white/60'}`} onClick={(e) => e.stopPropagation()}>
          ...
          <div className="mt-4 text-center"><span className={`text-[10px] ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{t.editHint}</span></div>
       </div>
    </div>
  );
};
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/20 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div className={`rounded-[32px] p-6 shadow-2xl w-72 transform transition-all scale-100 border ${containerClass}`} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={`mb-3 px-3 py-2 rounded-xl flex items-center gap-2 text-xs ${isBackupOverdue && lastBackupDate ? (isDark ? 'bg-red-900/20 text-red-400 ring-1 ring-red-900/50' : 'bg-red-50 text-red-600 ring-1 ring-red-200') : (isDark ? 'bg-slate-700 text-slate-300' : 'bg-slate-100 text-slate-500')}`}>
  ...
  return (
    ...
    >
      <div className={`w-full max-w-md shadow-2xl flex flex-col relative border transition-colors duration-300 h-auto min-h-[80vh] rounded-[40px] ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-white/60'}`} onClick={(e) => e.stopPropagation()}>
        ...
        {zoomedDateKey && <DayCardModal dateKey={zoomedDateKey} gridMode={gridMode} records={records} categories={categories} dayNotes={dayNotes} onClose={() => setZoomedDateKey(null)} onSaveNote={handleSaveDayNote} isDark={darkMode} t={t} />}
                          ...
                          {week.map((cell, dayIndex) => {
                               ...
                               subCells.push(
                                 <div 
                                    ...
                                    onClick={(e) => { 
                                        if (!isCurrent) return; 
                                        e.stopPropagation(); 
                                        handleCellClick(cell.dateKey, i, longPress.isLongPress.current); 
                                    }}
                                    className={`relative w-full h-full outline-none hoverable ${finalColor} hover:opacity-80`}
                                 >
                                    <div className={`absolute inset-0 pointer-events-none ${darkMode ? 'border-slate-600' : 'border-slate-300'} ${gridMode === 4 && i === 0 ? 'border-r-[0.5px] border-b-[0.5px]' : ''} ${gridMode === 4 && i === 1 ? 'border-b-[0.5px]' : ''} ${gridMode === 4 && i === 2 ? 'border-r-[0.5px]' : ''} ${gridMode === 6 && (i === 0 || i === 1) ? 'border-r-[0.5px] border-b-[0.5px]' : ''} ${gridMode === 6 && i === 2 ? 'border-b-[0.5px]' : ''} ${gridMode === 6 && (i === 3 || i === 4) ? 'border-r-[0.5px]' : ''}`}></div>
                                 </div>
                               );
                            }
                            ...
                            return (
                              ...
                              >
                                 ...
                                 {hasNote && (
                                     <div className={`absolute bottom-0.5 left-1/2 -translate-x-1/2 w-2 h-0.5 rounded-full ${darkMode ? 'bg-slate-400' : 'bg-slate-400'}`}></div>
                                 )}
                              </div>
                            );
                          })}
                    ...
                    return (
                      <div key={cat.id} className={`flex-1 w-full p-3 rounded-2xl border ${darkMode ? 'border-slate-700 bg-slate-800/40' : 'border-slate-200 bg-white/60'} shadow-sm flex items-center justify-between gap-4`}>
                         ...
                         <div className="flex-1 flex flex-col justify-center gap-1.5">
                            ...
                            <div className="flex justify-end items-center"><span className={`text-[9px] font-medium ${darkMode ? 'text-slate-500' : 'text-slate-400'}`}>{t.statsTotal}: {stats.totalCounts[cat.id]}{rangeInfo.hasData && <span className="opacity-70 ml-1">({rangeInfo.min} ~ {rangeInfo.max})</span>}</span></div>
                         </div>
                      </div>
                    )
                 })}
